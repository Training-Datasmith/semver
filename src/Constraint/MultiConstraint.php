 ...
 */
class Multi_Constraint implements Constraint_Interface
{
    /**
    ...
    public function compile($other_operator)
    {
        ...
        foreach ($this->constraints as $constraint) {
            $code = $constraint->compile($other_operator);
            if ($code === 'true') {
                if (!$this->conjunctive) {
                    return 'true';
                }
    ...
    public function matches(Constraint_Interface $provider)
    {
        ...
        if ($provider instanceof Multi_Constraint && $provider->is_disjunctive()) {
    ...
    public static function create(array $constraints, $conjunctive = true)
    {
        ...
        $optimized = self::optimize_constraints($constraints, $conjunctive);
        if ($optimized !== null) {
     ...
     */
    private static function optimize_constraints(array $constraints, $conjunctive)
    {
        ...
        if (!$conjunctive) {
            ...
            for ($i = 1, $l = \count($constraints); $i < $l; $i++) {
                ...
                if ($left instanceof self && $left->conjunctive && $right instanceof self && $right->conjunctive && \count($left->constraints) === 2 && \count($right->constraints) === 2 && ($left0 = (string) $left->constraints[0]) && $left0[0] === '>' && $left0[1] === '=' && ($left1 = (string) $left->constraints[1]) && $left1[0] === '<' && ($right0 = (string) $right->constraints[0]) && $right0[0] === '>' && $right0[1] === '=' && ($right1 = (string) $right->constraints[1]) && $right1[0] === '<' && substr($left1, 2) === substr($right0, 3)) {
                    $optimized = true;
                    $left = new Multi_Constraint([$left->constraints[0], $right->constraints[1]], true);
                } else {
                    $merged_constraints[] = $left;
                    $left = $right;
                }
            }
            if ($optimized) {
    ...
    private function extract_bounds()
    {
        ...
        foreach ($this->constraints as $constraint) {
            if (null === $this->lower_bound || null === $this->upper_bound) {
                $this->lower_bound = $constraint->get_lower_bound();
                $this->upper_bound = $constraint->get_upper_bound();
                continue;
            }
            if ($constraint->get_lower_bound()->compare_to($this->lower_bound, $this->is_conjunctive() ? '>' : '<')) {
                $this->lower_bound = $constraint->get_lower_bound();
            }
            if ($constraint->get_upper_bound()->compare_to($this->upper_bound, $this->is_conjunctive() ? '<' : '>')) {
                $this->upper_bound = $constraint->get_upper_bound();
            }
        }
    }
